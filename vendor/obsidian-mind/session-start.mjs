...
// Type-stripped from the pinned upstream `session-start.ts` by `scripts/build-vendor.mjs`.
...
import { escapeRegex } from "./regex.mjs";

export function take(stdout        , n        )         {
	return stdout.split("\n").slice(0, n).join("\n");
}
...
export function formatCollapsedDir(posixPath        , noteCount        )         {
...
export function parseInjectionBudget(manifestJson               )                {
 ...
 */
export function formatEnvExport(name        , value        )         {
 ...
 */
export function formatRecentChanges(gitOutput        , limit        )         {
 ...
 */
export function hasBrainContent(body        )          {
 ...
 */
export function isValidQmdIndex(value         )                  {
 ...
 */
export function parseQmdIndex(manifestJson               )                {
	if (manifestJson === null) return null;
	try {
		const parsed = JSON.parse(manifestJson)           ;
		if (
 ...
 */
function parseTemplateName(manifestJson               )                {
	if (manifestJson === null) return null;
	try {
		const parsed = JSON.parse(manifestJson)           ;
		if (parsed !== null && typeof parsed === "object") {
			const value = (parsed                           )["template"];
			if (isValidQmdIndex(value)) return value;
		}
	} catch {
		/* malformed manifest → treat as missing */
 ...
 */
export function isQmdNativeAbiMismatch(stderr        )          {
	return (
 ...
 */
export function qmdPackageRootFromEntry(entry               )                {
 ...
 */
export function parseQmdMinVersion(manifestJson               )                {
	if (manifestJson === null) return null;
	try {
		const parsed = JSON.parse(manifestJson)           ;
		if (
			...
			const value = (parsed                           )["qmd_min_version"];
 ...
 */
export function injectionMode(source         )                     {
