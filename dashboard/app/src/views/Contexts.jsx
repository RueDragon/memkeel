...
export default function Contexts({ model, openDetail, reload }) {
  ...
  const columns = [
    { title: t('col.id'), dataIndex: 'id', width: 230, render: (v) => <span className="mono">{v}</span> },
    { title: t('col.workspace'), dataIndex: 'workspace', width: 180, render: (v) => <WorkspaceLink value={v} openDetail={openDetail} /> },
    { title: t('col.task'), dataIndex: 'task', width: 220 },
    { title: t('col.contextText'), dataIndex: 'text', width: 480, render: (v, r) => <ChatPreview task={r.task} text={v} host={r.agent} /> },
    ...
    <div className="table-view">
      <div className="panel">
        <DataTable
          ...
          data={model?.contexts ?? []}
          rowKey={(r) => r.id}
          searchPlaceholder={t('contexts.filter')}
