...
`doctor` 会报告配置路径、缺失的预期文件、尚未生成的已注册主题笔记、待处理的 capture、检查点
健康状况，以及**三把**写入锁（存储锁、hook 队列锁与 setup 锁）及其持有者的存活状态。当确实出现
问题时，它以非零退出。

"还在等什么"被分成三类报告，因为它们需要三种不同的应对：

- `pending` 统计排队中的工作：已记录但尚未消费的事件、计划了却始终没写下来的 capture、没有
  正常结束的检查点会话，以及尚未生成的主题笔记；
- `recovery` 把上述每一项变成"下一步该做什么"；
- `ledger` 说明事件日志究竟能不能被读取。读不出来的状态会以 `corrupt` 列出，并写明需要人做什么，
  而不是被混进"积压"里。

已注册但还不存在的主题笔记既不是损坏，也不会让这次运行失败：主题有了事件之后，consolidate 会
写出它；如果它被删掉了，`maintenance --rebuild` 可以恢复。两者是**故意**分开的：当待处理的工作
被残留的写入锁挡住时，提示会先让你去处理那把锁，而不会声称下一次运行会自行消化积压——只要那把锁
还在，每个写入者都会拒绝进入。`doctor` 不会改动它报告的任何东西：不清锁、不修检查点、也不重写
它读不出来的笔记。

写入锁不会被自动移除，即使其记录的持有者可以确认已经不在了：`doctor` 会把这种情况报告为
stale，让诊断可见，被阻塞的写入也会在错误里给出同一份持有者信息以及需要删除的文件，但删不删
由你决定——先停掉所有写入者（包括任何会让它们重新启动的东西，例如会重新拉起进程的宿主 hook），
再自行删掉那个锁文件，而不是让恢复中的进程去猜一个它看不见的持有者是否真的结束了。锁只是一个
各写入者约定遵守的文件：它能挡住另一个同样来取锁的进程，但挡不住一个根本不取锁就直接改写存储
的程序。这里权衡的另一面是两个写入者同时进入账本，而那是更糟的失败。
...
- `create` 拒绝覆盖已存在的笔记，随后发布字节并回读。
- `append` 以原子方式发布整篇笔记：会回读，不一致会被**报告**而不是被"修复"—— 字节本身已经
  完整，把之前的字节写回去等于删掉一次已经成功的追加。如果追加的块已经是笔记尾部，就不会再写
  一次，因此被打断后重试是安全的。
- `replace` 由期望值守卫，会在 `backups/replacements/` 下写入一个带时间戳、含前后哈希的
  备份，并重新检查目标在预检与写入之间没有发生变化。

以上每次写入 —— 以及 `obsidian-cli` 模式下的同类写入 —— 都是一次**原子发布**：字节先写入同目录
的临时文件（`<name>.<uuid>.tmp`），flush 之后 rename 覆盖目标。进程在任何一点被杀，留下的都是
原来那篇完整笔记，而不是半篇；重试是安全的。这**不**声称的边界：临时文件自身的字节在 rename 前
用 `fsync` 刷过，但目录项没有刷，Windows 也没有刷目录项的手段，所以断电可能丢掉最新一次写入 ——
但不会让文件变成半篇。被 kill 留下的残留文件不会被自动删除，因为后续运行无法证明自己拥有该文件；
它的名字是 `.tmp`，笔记扫描器从不读取，因此永远不会变成一个事件。目标位置若是符号链接，会解析到
它指向的真实文件并在那里发布，链接得以保留、写入仍然是原子的；断链、指向目录的链接、以及解析到
存储之外的链接都会被拒绝。
...
恢复是把存储带回来，迁移是把现役存储搬走。两者形态相同 —— 复制、校验、改指 —— 所以共用同一套"什么必须携带"的分类，以及同一条"配置写在哪一步"的规则。

| 命令 | 作用 |
| --- | --- |
| `migrate --to DIR` | 只读计划：报告来源、目标、文件数与字节数、将被改写的配置键，以及当前生效的工作区别名。不写任何东西。 |
| `migrate --to DIR --execute` | 把 home 与存储复制到 `DIR/home` 与 `DIR/store`，逐文件按读取时的哈希校验，并**最后**写入改指后的 `config.json`。拒绝与来源重叠、位于 memkeel 代码库内部、或已有数据的目标。 |

```bash
memkeel migrate --to /srv/memkeel            # 只出计划
memkeel migrate --to /srv/memkeel --execute
MEMKEEL_HOME=/srv/memkeel/home memkeel doctor
```

有三条性质值得写清楚，因为每一条都对应一种出错方式：

- **来源永不被修改或删除。** 迁移就是"复制 + 改指"。删除旧的 home 是独立且需要人有意识执行的动作，命令会把它打印出来，而不会因为本次执行成功就替你推断。
- **配置最后写入。** `config.json` 被完全排除在复制循环之外，而不是"先复制、稍后改写"：因此中途失败留下的目标**根本没有配置**，不可能被误当成一个可用的 home。若目标里的配置仍指向旧存储，那看起来像迁移完成，实际读的却是原来的存储。
- **已记录的证据不被改写。** 事件保留它写入时的证据路径。工作区别名原样携带，因为别名正是"项目目录搬走后仍解析到同一个工作区"的机制；账本记录的是发生过什么，不是一组实时链接。

### 采集与隐私

「关闭采集」就是真的关闭。`config.json` 里的 `collection` 由**每一个会写入会话正文的层**读取，而不是由展示它的页面读取：

| 层 | 开关做了什么 |
| --- | --- |
| Hook 队列 | 不写入队列，正文根本不会进入 `state/hook-queue/`。 |
| 会话状态文件 | 提示词与最近回复不再写入 `state/hook-sessions/`，`factRecall` 里的召回 query 也不再写入。 |
| 检查点转证据 | 在开关关闭**之前**已入队的检查点会被 `held`（暂缓），而不是被提升为证据；开关重新打开后这些行会正常消化。 |
| 访问日志 | 选择退出的工作区不再追加任何条目。 |
| 历史 ingest | `ingest-apply` 会跳过工作区、宿主或路径不在采集范围内的候选。 |
...
| `privacy cleanup [--host H] [--workspace W] [--cwd DIR]` | 只读预览。会列出保留清理**将要删除的每一个路径**，带上所属分组、实测大小与判定所依据的修改时间，并明确列出不会碰的东西。不删除任何东西。 |
| `privacy cleanup --execute` | 只删除计划里列出的那些路径。有四类东西在构造上就够不着：事件账本与存储内容（保留策略从不改写不可变记录）；你自己用 `backup create --out` 写在别处的归档（那个目录属于你，本程序不知道它在哪里）；`backups/replacements/` —— 原子写入所依赖的回滚目录，按路径看像备份、其实不是；以及 memory home 之外的一切，因为每个目标在删除前都会从 home 重新推导并重新校验，而不是信任计划里的字符串。 |
...
设置页（dashboard 的 **Settings**）会把这同一份策略**只读展示**出来 —— 判定结果、各作用域状态、排除规则，以及上面那三种状态 —— 数据来自与 CLI 同一个函数，因此页面与 `privacy show` 不可能说法不一致。它**刻意不做成编辑器**：关闭采集会影响 hook、ingest 与访问日志，所以页面报告的是「现在生效的是什么」，而不是给一个看起来像开关、实际只管渲染的控件。由于页面没有具体会话上下文，它会把判定标注为「未指定宿主 / 工作区时的默认判定」，并单独列出各作用域 —— 工作区级的关闭对"没有指定任何工作区"的问题本来就不适用，页面如实说明，而不是含糊过去。
...
作用域的优先级设计成「只会越来越私密」，不会反过来：

1. `enabled: false` 是硬停。下面的任何一项都不能把它重新打开。一个能被过期的单宿主配置覆盖的全局关闭开关，是陷阱而不是功能。
2. 命中的排除规则优先。显式拒绝是用户能做出的最具体的表态。
3. `workspaces[<id>] === false`。
4. `hosts[<host>] === false`。

`true` 值可以写，用来表达意图，但它永远不会覆盖更宽的拒绝。`collection` 整段缺失或格式错误一律视为**采集** —— 因此已有存储在开关出现之前是怎么工作的，之后就还是怎么工作，而且一个拼写错误绝不会静默地停止采集。

匹配是精确的，每一类都说清自己的含义：`paths` 规则匹配该目录本身及其内部所有内容（规则是 `work` 时不会匹配到 `workshop`），`sessionTypes` 与 `sources` 整值匹配。没有通配符，也没有子串意外。

**有三件不同的事都被叫做「删除」，所以它们被分开命名** —— 写在返回结果里，而不只是写在这里：

- **不采集** —— 这段会话从未被写入。没有队列条目、没有检查点、没有证据、没有访问日志。无需撤销，因为什么都没发生。
- **软删除 / 不参与检索** —— 原始记录仍完整存在于账本中，只是不再进入默认摘要与普通召回。历史与来源仍可追溯。
- **物理删除** —— **不提供。** 账本只追加、每个事件都带校验，就地删除会破坏它自身的完整性，也无法收回已经产生的备份、快照与外部同步副本。替代做法是：停止采集、在采集**之前**就排除敏感路径、并让保留策略处理派生材料 —— 具体范围见 `memkeel privacy cleanup`。
...
| `doctor` | 健康检查：配置与预期文件、尚未生成的主题投影、待处理工作及清除每一项的下一步、读不出来的内容（corrupt），以及三把写入锁及其持有者的存活状态。只读：不改动它所报告的任何东西。 |
...
而文件系统保持为写入通道。出于同样的原因，每次写入都是原子发布：直接写入会先截断目标，
否则一次损坏或被中断的追加就会让整个存储的 `loadEvents` 失效。
...
- **`setup` 会保留一份带版本号的安装记录**，位于 `<memory home>/state/setup-receipt.json`：逐文件
  记录安装前的原始字节、它最近写入的字节，以及写入进行中时**打算写入**的字节。`setup --uninstall`
  依赖它恢复，并会拒绝安装之后被改动过的文件，所以请把它当作私密数据、不要手工编辑。中断后的恢复
  被有意分成两类：宿主文件里仍是那次被中断的运行所写入的字节时，重新运行 `setup` 会直接把它认作已
  安装状态；仍是那次运行之前记录的字节时，这次未完成的写入会被丢弃——首次安装如果根本没写到文件，
  它的记录条目会被整条删除，因为没有任何东西需要恢复；两者都不是时，程序不做任何猜测、不写任何
  字节：它会拒绝该文件、在错误里点名并说明当时有写入正在进行，由你依据记录里的安装前原始字节或
  setup 备份手工恢复。每次运行还会说明自己做了什么：`first-install`、`no-change`、`upgrade`、
  `rebind`、`refresh` 或 `uninstall`。
