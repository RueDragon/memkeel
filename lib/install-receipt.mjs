...
export function readInstallReceipt(home) {
  ...
  for (const [name, row] of Object.entries(raw.files)) {
    // A row without both byte strings cannot restore anything; keeping it would make an uninstall
    // believe it owns a file it has no original for.
    if (row && typeof row === 'object' && typeof row.after === 'string' && (row.before === null || typeof row.before === 'string')) files[name] = { label: row.label ?? null, before: row.before ?? null, after: row.after };
  }
