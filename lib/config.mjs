...
export function applyConfigMigration(home, { backupDir } = {}) {
  ...
  if (!plan.changes.length) {
    return { applied: false, file, backup: null, changes: [], reason: `配置已经是 schema ${CONFIG_SCHEMA_VERSION}，无需迁移。` };
  }
  ...
  if (!report.ok) {
    ...
    const error = new ConfigError(`迁移后的配置校验未通过，未写入任何内容：${report.issues.map((issue) => issue.message).join('；')}`);
