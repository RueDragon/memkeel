...
export function readConfigFile(config) {
  ...
  catch (error) { throw messageError('cli.config.error.notJson', { file, error: error.message }); }
}
...
export function applyConfigMigration(home, { backupDir } = {}) {
  ...
  if (fs.readFileSync(file, 'utf8') !== before) {
    throw messageError('cli.config.error.changedDuringMigration', undefined, ConfigError);
  }
  ...
  try { written = fs.readFileSync(file, 'utf8'); }
    ...
    throw messageError('cli.config.error.readBackFailed', { error: error.message }, ConfigError);
  }
  if (JSON.stringify(JSON.parse(written)) !== JSON.stringify(plan.next)) {
    fs.writeFileSync(file, before, 'utf8');
    throw messageError('cli.config.error.writeMismatchMigration', { backup }, ConfigError);
  }
