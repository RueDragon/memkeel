...
export function loadTranscript({ host, sessionId, transcriptPath, cwd }, { limit = 400 } = {}) {
  ...
  if (!locator) return { available: false, reason: `未知宿主：${host || '空'}`, source: '', turns: [] };
  let file = '';
  try { file = locator(sessionId, transcriptPath, cwd); } catch (error) { return { available: false, reason: `定位失败：${error.message}`, source: '', turns: [] }; }
  if (!file || !fs.existsSync(file)) {
    return { available: false, reason: sessionId ? '未找到该会话的真实记录文件' : '该会话未记录 session id（旧记录）', source: '', turns: [] };
  }
  const info = fileInfo(file);
  if (!info) return { available: false, reason: '真实记录文件不可读', source: file, turns: [] };

  if (host === 'dsh' || file.endsWith('.zstd')) {
    const decoded = readZstd(file);
    if (decoded.error) {
      return { available: false, reason: decoded.error.startsWith('NO_ZSTD:') ? '缺少 Python zstandard 模块，无法解压 dsh 记录' : `解压失败：${decoded.error}`, source: file, turns: [] };
    }
  ...
  let raw;
  try { raw = fs.readFileSync(file, 'utf8'); } catch (error) { return { available: false, reason: `读取失败：${error.message}`, source: file, turns: [] }; }
