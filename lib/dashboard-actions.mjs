...
import { DEFAULT_LOCALE, makeTranslator, messageError, msg, renderMessages } from './messages.mjs';
...
// The validator hands back message references. An Error can only carry a string, so a refusal
// renders its issues in the default locale for `.message` and hands the references to the caller
// separately: the settings page renders those in the reader's own language, which the server
// cannot choose on its behalf.
const renderDefault = makeTranslator(DEFAULT_LOCALE);
...
export function planCloseAction(config, projection, { topic, actionId }) {
  ...
  return {
    kind: 'close-action',
    summary: msg('cli.plan.closeAction', { text: action.text }),
...
export function planHabitDecision(config, projection, { candidateEvent, preferenceId, decision, userQuote = null }) {
  ...
  return {
    kind: 'habit-decision',
    summary: msg('cli.plan.habitDecision', { verb: msg(decision === 'confirmed' ? 'cli.plan.verb.confirm' : 'cli.plan.verb.reject'), text: candidate.text }),
...
export function planReviseFact(config, projection, { topic, key, text, expectedEvent, retire = false, supersedeEvent = null }) {
  ...
  return {
    kind: 'revise-fact',
    summary: msg('cli.plan.fact', { verb: msg(retire ? 'cli.plan.verb.retire' : 'cli.plan.verb.revise'), topic, key, text: body.slice(0, 60) }),
...
export function planReviseLearning(config, projection, { type, topic, id, text, expectedEvent, retire = false, keep = {} }) {
  ...
  return {
    ...
    summary: msg('cli.plan.learning', { verb: msg(retire ? 'cli.plan.verb.retire' : 'cli.plan.verb.revise'), kind: msg(`cli.plan.kind.${type}`), id, text: body.slice(0, 60) }),
...
export function planReviseAction(config, projection, { topic, actionId, text, retire = false }) {
  ...
  return {
    kind: 'revise-action',
    summary: msg('cli.plan.action', { verb: msg(retire ? 'cli.plan.verb.close' : 'cli.plan.verb.revise'), text: current.text.slice(0, 60) }),
...
export function planRevokeHabit(config, projection, { preferenceId }) {
  ...
  if (!active) {
    throw messageError('cli.pref.manualOnly');
  }
  ...
  if (!previous) throw messageError('cli.pref.noPrevious');
  ...
  return {
    kind: 'revoke-habit',
    summary: msg('cli.plan.revokeHabit', { text: active.text }),
...
export function planComposeEvent(config, { kind, topic, workspace, text, key, task, evidence, triggers, verification, scope, ttlDays, certainty, supersedes, agent = 'dashboard' }) {
  ...
  return {
    kind: 'compose-event',
    summary: msg('cli.plan.compose', { kind: msg(`cli.plan.newKind.${kind}`), text: text.trim().slice(0, 60) }),
    changes: [{ field: kind, from: msg('cli.plan.value.none'), to: text.trim().slice(0, 60) }],
    event,
  };
}
...
function refused(issues) {
  const refs = issues.map((issue) => issue.message);
  const error = new Error(refs.map((ref) => renderMessages(ref, renderDefault)).join('; '));
  error.issues = refs;
  return error;
}
...
export function planUpdateConfig(config, input = {}) {
  ...
  if (!changes.length) throw refused([{ field: 'config', message: msg('cli.config.error.noChanges') }]);
  // The summary is a message reference rather than a sentence: it is shown in the confirmation
  // dialog, never written to the ledger (`executeUpdateConfig` records no event), so the page can
  // word it in the reader's language. The fields that change are listed separately in `changes`.
  return {
    kind: 'update-config',
    summary: msg('cli.config.summary.update', { n: changes.length }),
    changes,
    configPath: file,
...
function executeUpdateConfig(config, transport, plan) {
  ...
  catch (error) { throw messageError('cli.config.error.readFailed', { file, error: error.message }); }
  ...
  if (sha(before) !== plan.expectedConfigHash) throw messageError('cli.config.error.changedAfterPreview');
  ...
  catch (error) { throw messageError('cli.config.error.readBackFailedAfterWrite', { file, error: error.message }); }
  if (JSON.stringify(written) !== JSON.stringify(next)) {
    fs.writeFileSync(file, before, 'utf8');
    throw messageError('cli.config.error.writeMismatch');
  }
  return {
    kind: 'update-config',
    configPath: file,
