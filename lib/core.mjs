...
// The `.mjs` is generated from the vendored `.ts` by `scripts/build-vendor.mjs`. Importing the `.ts`
// directly works from a checkout and fails for everyone who installs the package, because Node refuses
// to strip TypeScript types under `node_modules`; see THIRD_PARTY.md and RELEASE.md.
import { applyInjectionBudget, formatInjectionSize } from '../vendor/obsidian-mind/session-start.mjs';
...
// One formatter for the whole process: constructing an Intl.DateTimeFormat per call costs ~55 us,
// which is what made every localDay call site a hot path once the ledger grew.
...
// The parsed index, kept between calls.
//
// A cached refresh used to re-parse the whole file every time, and that was the remaining cost even
// after the write was made conditional: at 10k events the document is ~15 MiB and every entry carries
// its note's lowercased text, so the parse alone measured 60.9 ms per call while costing nothing.
//
// The cache is keyed on the file's own path, mtime and size, so an index written by another process
// (or restored from a backup) is picked up rather than served stale. Only one file is remembered: two
// stores alternating in one process would re-parse each time, which is slower but never wrong.
//
// The `entries` handed out are therefore the *same* objects across calls, so callers must treat them
// as read-only. Every caller does today — they use `Object.values`, filters and property reads — and
// that is what makes sharing safe here rather than merely convenient.
let indexCache = null;
function indexFileKey(file) {
...
export function refreshIndex(config, { force = false, persist = true } = {}) {
  ...
  let old = { entries: {} };
  const key = indexFileKey(file);
  if (indexCache && indexCache.key === key) old = indexCache.index;
  else {
    if (fs.existsSync(file)) { try { old = JSON.parse(fs.readFileSync(file, 'utf8')); } catch { /* Rebuild corrupt cache. */ } }
    indexCache = { key, index: old };
  }
  ...
  if (persist && !current) {
    ...
    // What was just written is what is now on disk, so remember it under the new key rather than
    // letting the next call re-parse this function's own output.
    indexCache = { key: indexFileKey(file), index: result };
  }
  return result;
}
...
export function consolidate(config, transport, { failBeforeCheckpoint = false, rebuild = false } = {}) {
  return withLock(path.join(config.policyRoot, 'state'), () => {
    ...
    // Bucket the visible events by day in one pass: the previous per-day `visible.filter` re-derived
    // every event's day for each pending day, so a single pending event still cost O(visible).
    const rowsByDay = new Map([...new Set(pending.map((event) => localDay(event.recorded_at)))].map((day) => [day, []]));
    for (const event of visible) {
      const rows = rowsByDay.get(localDay(event.recorded_at));
