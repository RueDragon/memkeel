...
import { DEFAULT_LOCALE, makeTranslator, renderMessages } from './messages.mjs';
import { atomicJson, withLock } from './transport.mjs';

const REPOSITORY_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');

// The validator hands back message references, and this module's own sentences are English. A
// migration refusal therefore renders them in the default locale: it is a string in an Error, and
// printing the reference itself would show "[object Object]" to whoever reads the log.
...
export function executeMigration(config, plan, { version = '', now = new Date().toISOString() } = {}) {
  if (!plan?.ok) throw new Error(`Refusing to migrate: ${(plan?.issues ?? []).map((issue) => renderMessages(issue.message, renderDefault)).join('; ')}`);
  ...
  return withLock(path.join(config.policyRoot, 'state'), () => {
    ...
    if (!report.ok) {
      ...
      fs.rmSync(configFile, { force: true });
      throw new Error(`The migrated configuration does not validate, so nothing was switched: ${report.issues.map((issue) => renderMessages(issue.message, renderDefault)).join('; ')}`);
    }
