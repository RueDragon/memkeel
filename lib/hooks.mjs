  ...
  return withLock(root, () => {
    ...
    if (call.event === 'SessionStart') {
      ...
      if (state.promptDigest !== promptDigest || state.lastStop) {
        state.turn++; state.promptDigest = promptDigest; state.prompt = redact(call.prompt, 900); state.lastStop = false;
        state.readOnly = /不(?:要)?(?:记录|保存|写收尾|写入记忆|写入)|禁止.*写入|只读(?:验收|测试)|do not (?:save|record)|no memory writes/i.test(call.prompt);
        if (state.readOnly) state.prompt = '';
      ...
      state.factRecall = { query: redact(call.prompt, 160), matched: facts.map((f) => ({ topic: f.topic, key: f.key, event_id: f.event_id, crossWorkspace: f.crossWorkspace })), at: state.updatedAt };
      ...
      const assistant = state.readOnly ? '' : supplied || state.lastAssistant || '';
      if (!state.readOnly && assistant) state.lastAssistant = assistant;
      ...
      if (!state.readOnly && state.prompt && (assistant || state.tools > 0)) {
