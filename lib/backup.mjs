...
import { atomicJson, isInsidePath, withLock } from './transport.mjs';
...
// `isInsidePath` rather than a second copy of the same rule: a backup that refused a destination and a
// write that allowed it must not be able to disagree about where the boundary is.
...
/**
 * The entries a copy of this configuration would carry, with their hashes.
 *
 * `includeDerived` separates the two callers: an archive leaves the rebuildable files out, because
 * `memkeel index` recreates them and carrying them only makes the archive bigger, while a migration
 * carries them, because the point of a migration is that the destination is immediately equivalent
 * rather than equivalent after a rebuild.
 */
export function collectEntries(config, { includeDerived = false } = {}) {
  const entries = [];
  const skipped = [];
  for (const row of walk(config.policyRoot)) {
    ...
    if (kind === 'skip') { skipped.push({ path: `home/${row.relative}`, reason: 'runtime state' }); continue; }
    if (kind === 'derived' && !includeDerived) { skipped.push({ path: `home/${row.relative}`, reason: 'rebuildable' }); continue; }
    entries.push({ source: row.full, path: `home/${row.relative}`, kind, bytes: fs.statSync(row.full).size, sha256: sha256File(row.full) });
  }
  ...
  if (fs.existsSync(configFile)) {
    ...
    if (changes.length) {
      try {
        ...
        // A configuration that still names the store it was restored from is the one outcome this
        // rewrite exists to prevent - the destination would look restored and read the source data -
        // so a failed rewrite leaves no configuration at all rather than a wrong or partial one.
        try { fs.rmSync(configFile, { force: true }); } catch { /* nothing to remove */ }
        throw error;
      }
      if (JSON.parse(fs.readFileSync(configFile, 'utf8')).vaultRoot !== storeDir) {
        fs.rmSync(configFile, { force: true });
        throw new Error(`The restored configuration could not be read back, so it was removed rather than left pointing at the source store: ${configFile}`);
      }
