...
import { privacyView } from './privacy.mjs';
...
const OBSIDIAN_LOCATIONS = [
  [msg('cli.obsidian.label.windowsUser'), ({ home, env }) => path.join(env.LOCALAPPDATA ?? path.join(home, 'AppData', 'Local'), 'Obsidian', 'Obsidian.exe')],
  [msg('cli.obsidian.label.windowsGlobal'), ({ env }) => path.join(env.ProgramFiles ?? 'C:\\Program Files', 'Obsidian', 'Obsidian.exe')],
  [msg('cli.obsidian.label.macosApp'), () => '/Applications/Obsidian.app'],
  [msg('cli.obsidian.label.linuxSystem'), () => '/usr/bin/obsidian'],
  [msg('cli.obsidian.label.userLocalBin'), ({ home }) => path.join(home, '.local', 'bin', 'obsidian')],
];
...
export function obsidianStatus(config, { home = os.homedir(), env = process.env } = {}) {
  ...
  const detected = [];
  if (configuredPath) detected.push({ label: msg('cli.obsidian.label.configured'), path: configuredPath, exists: fs.existsSync(configuredPath) });
  ...
  for (const file of pathObsidian({ env })) detected.push({ label: msg('cli.obsidian.label.pathFound'), path: file, exists: true });
  return {
...
export const RESTART_NOTICE = Object.freeze({
  required: true,
  reason: msg('cli.restart.reason'),
  processes: [
    { id: 'codex', label: 'Codex', service: msg('cli.restart.service', { name: MCP_SERVER_NAME }) },
    { id: 'claude', label: 'Claude Code', service: msg('cli.restart.service', { name: MCP_SERVER_NAME }) },
    { id: 'zcode', label: 'ZCode', service: msg('cli.restart.service', { name: MCP_SERVER_NAME }) },
    { id: 'dsh', label: 'dsh', service: msg('cli.restart.servicePerProfile', { name: MCP_SERVER_NAME }) },
  ],
  command: 'memkeel setup --check',
  commandHint: msg('cli.restart.commandHint'),
...
export function settingsSnapshot(config, { home = os.homedir(), env = process.env } = {}) {
  ...
  // Which of the editable fields this configuration file actually sets, and which are in force because
  // the program fell back to its own choice. The page shows the effective value either way; this is what
  // lets it say *why* that value is what it is, which is the difference between "your setting" and "the
  // default" — and the difference a user needs before deciding whether changing it will do anything.
  //
  // Only the origin is reported, deliberately not a default *value*: several fallbacks are described in
  // prose ("unlimited unless configured", "14 days at runtime") rather than as a single literal, and
  // inventing a number here would be a second source of truth for something the validator already owns.
  const fields = ['storage', 'memoryRoot', 'vaultRoot', 'vaultName', 'obsidianCli', 'layout',
    'activeLimit', 'recentLimit', 'recentDays', 'budgetBytes'];
  const provenance = {};
  for (const key of fields) provenance[key] = Object.prototype.hasOwnProperty.call(raw, key) && raw[key] !== undefined ? 'config-file' : 'fallback';
  for (const [role] of CONFIG_ROLE_FIELDS) {
    ...
    const setByFile = (raw.roles && raw.roles[role] !== undefined) || raw[role] !== undefined;
    provenance[`roles.${role}`] = setByFile ? 'config-file' : 'fallback';
  }
  ...
  return {
    ...
    // The effective collection policy, shown rather than edited: turning collection off is a config
    // decision with consequences outside this page (hooks, ingest, the access log), so the page
    // reports what is in force and who decided it instead of offering a switch that only looks like
    // one. `privacyView` is the same function the CLI prints, so the two cannot disagree.
    collection: privacyView(config),
