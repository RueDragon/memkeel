...
export function checkpointHealth(config, events) {
  const pending = []; const unqueued = []; const invalid = []; const missingEvents = []; const uninitialized = [];
  ...
  return { healthy: !pending.length && !unqueued.length && !invalid.length && !missingEvents.length, pending, unqueued, invalid, missingEvents, uninitialized };
}
