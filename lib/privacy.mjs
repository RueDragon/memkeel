...
export function cleanupPreview(config, context = {}) {
  ...
  return {
    ...
    scope: [
      ...
      { target: '备份归档', rule: `仅保留最近 ${collection.retention.backups} 份`, effect: '只清理 memory home 内的 setup 快照与 config-migrations 备份；你自己用 backup create --out 写出的归档不在范围内，也永远不会被本命令碰到' },
      { target: '会话运行时状态与启动诊断', rule: `超过 ${collection.retention.diagnosticsDays} 天未更新即清理`, effect: 'state/hook-sessions/ 与 state/last-bootstrap.json；两者都是可重建的运行时状态，不属于账本' },
    ],
    notCovered: [
      '已写入账本的事件（不可变、只追加）。',
      '已经复制出去的备份、快照与外部同步副本。',
      '宿主自己的会话记录文件——它们由宿主管理，本程序只读不写。',
      'backups/replacements/：原子写入的回滚目录，按路径看像备份、其实不是，任何一个正在进行的写入都可能依赖它。',
    ],
    note: '只读预览：没有删除任何文件。',
  };
}
