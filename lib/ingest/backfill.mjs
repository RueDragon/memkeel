...
export function planCodexBackfill(config, { root, since = null, limit = Infinity, autoRegister = false } = {}) {
  ...
  for (const candidate of collected.kept) {
    ...
    // Historical ingest honours the collection switch too. It is the layer most likely to import
    // text from *before* the user turned collection off, so leaving it ungated would quietly
    // reintroduce exactly the material the switch was meant to keep out.
    if (!resolveCollection(config, { host: candidate.harness, workspace: route.id, cwd }).collecting) { skippedByCollection += 1; continue; }
...
export function applyCodexBackfill(config, transport, { root, since = null, limit = Infinity, autoRegister = false, onProgress } = {}) {
  ...
  for (const candidate of collected.kept) {
    ...
    // Same gate as the plan, so a rehearsal and a run cannot disagree about what is in scope.
    if (!resolveCollection(config, { host: candidate.harness, workspace: discovered.id, cwd }).collecting) { skippedByCollection += 1; continue; }
  ...
  return { mode: 'apply', written: written.length, skippedExisting, skippedByCollection, errors, planned: collected.kept.length, skipped: collected.skipped.length };
}
