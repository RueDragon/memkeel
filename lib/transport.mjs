 ...
 *
 * `relative` is validated by `inside`, which rejects a relative path that escapes and refuses a link
 * whose real path does. A symlink at the target path itself is then resolved, and the bytes are
 * published at what it points to - so the write stays atomic and the link survives.
 *
 * The check that follows the resolution is not a second opinion on the same question. `inside` answered
 * for the path as it was when it ran, and the link is followed again here, so a link swapped in between
 * the two would otherwise send the publish outside the store. It narrows that window rather than closing
 * it - the check and the rename are still not one operation - and no test can produce that swap
 * deterministically, so this is defence in depth: the refusals a user meets come from `inside`, or from
 * the broken-link and link-to-a-directory cases in `resolveWritableTarget`.
 */
