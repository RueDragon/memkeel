...
/** The raw in-place write: the bytes go straight into the target, truncated first. */
function writeInPlace(file, content, { encoding = 'utf8', mode = null } = {}) {
 ...
 *
 * The replacement is all-or-nothing. A direct write truncates the target before the new bytes exist,
 * so a process killed in between leaves half of the new content where a configuration used to be -
 * and the installation receipt would then hold `before` bytes that match nothing on disk, turning a
 * failed write into a state a human has to resolve. A rename either happens or does not.
 *
 * A symlink is the exception: rename would replace the link itself with a regular file, so a link is
 * written through in place, which keeps a dotfile layout the user set up on purpose.
 */
export function writeFilePreservingMode(file, content, { encoding = 'utf8', modeFrom = file } = {}) {
  const mode = existingMode(modeFrom);
  let link = false;
  try { link = fs.lstatSync(file).isSymbolicLink(); } catch { /* a target that does not exist yet */ }
  if (link) { writeInPlace(file, content, { encoding, mode }); return file; }
  const temp = `${file}.${crypto.randomUUID()}.tmp`;
  try {
    writeInPlace(temp, content, { encoding, mode });
    fs.renameSync(temp, file);
  } catch (error) {
    // A failed write must not leave a partial file, or its temporary, in the directory it was
    // rewriting - that directory holds another application's configuration.
...
export function atomicJson(file, value) {
  ...
  const temp = `${file}.${crypto.randomUUID()}.tmp`;
  try {
    // The temporary is written in place on purpose: it is already private and the rename that follows
    // is what makes the replacement atomic.
    writeInPlace(temp, `${JSON.stringify(value, null, 2)}\n`, { mode: existingMode(file) });
    fs.renameSync(temp, file);
  } catch (error) {
    // A failed write must not leave a partial JSON file beside the real one: the next reader would
    // have to tell it apart from the file it was replacing.
    try { fs.rmSync(temp, { force: true }); } catch { /* nothing to clean up */ }
    throw error;
  }
}
