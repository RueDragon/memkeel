...
// human who has confirmed that no writer is running deletes the file and retries. Paying a manual
// step after a crash is the price of never letting two writers into the ledger, and the refusal
// below names the exact file to remove so that recovery is a decision, not a guess.
...
function blockedMessage(file, owner) {
  ...
  const recovery = 'This lock is never removed automatically, because a stale record cannot be told apart from a live holder whose lock is being replaced right now. Confirm that no writer is running, delete that file yourself, and retry; "node memory.mjs doctor" reports the same facts without writing anything.';
