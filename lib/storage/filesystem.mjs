...
import { alreadyAppended, atomicJson, inside, publishStoreFile, sha } from '../transport.mjs';
...
// every note is published by a rename so a killed process cannot leave a partial one,
// and replacements are guarded by an expected value.
export class FilesystemTransport {
  ...
  create(relative, content) {
    ...
    // Published all at once, so a process killed mid-write cannot leave a partial note. The readback is
    // a report rather than a reason to delete: the bytes are already whole, and removing a complete note
    // because a *view* of it disagrees would destroy what the check exists to protect.
    publishStoreFile(this.config.vaultRoot, relative, content);
    if (fs.readFileSync(target, 'utf8') !== content) throw new Error(`Create content mismatch: ${relative}`);
    return target;
  }
  ...
  append(relative, content) {
    ...
    // No rollback on a mismatch: the publish is atomic, so a mismatch means something else changed the
    // file, and writing the previous bytes back would overwrite whatever that was.
    if (fs.readFileSync(target, 'utf8') !== next) throw new Error(`Append mismatch: ${relative}`);
  }
  ...
  replace(relative, expected, next) {
    ...
    atomicJson(path.join(backupDir, `${id}.json`), { relative, before: sha(current), after: sha(next) });
    if (fs.readFileSync(target, 'utf8') !== current) throw new Error('Target changed after preflight');
    // The pre-image is on disk and the target is unchanged, so publishing now is all-or-nothing.
    publishStoreFile(this.config.vaultRoot, relative, next);
